...
my %per_version;
my @function_sets;
my $xs_boot= qq{  HV* stash= gv_stashpv("Linux::EvDev", GV_ADD);\n};
my %xs_ctor= (
    i => 'newSViv(%s)',
    u => 'newSVuv(%s)',
    n => 'newSVnv(%s)',
...
for (sort { $a->{min} cmp $b->{min} or $a->{max} cmp $b->{max} } values %per_version) {
    ...
    my @max= split /\./, $_->{max};
    if (@min || @max) {
		die "TODO: detect version of libevdev";
        #$xs_boot .= "#if (";
        #if (@min) {
    ...
    if (@min || @max) {
        $xs_boot .= "#else\n";
        for my $item (sort @{ $_->{list} }) {
            $xs_boot .= sprintf("  newXS(\"Linux::EvDev::%s\", XS_Linux__EvDev__const_unavailable, file);\n", $item->{name});
        }
        $xs_boot .= "#endif\n";
    }
}
...
patch_file("EvDev.xs",           'GENERATED BOOT CONSTANTS', $xs_boot);
patch_file("lib/Linux/EvDev.pm", 'GENERATED XS CONSTANT LIST', $consts_pl);
#patch_file("lib/Linux/EvDev.pm", 'GENERATED XS FUNCTION LIST', $fn_pl);
